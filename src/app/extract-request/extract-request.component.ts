...
import { AfterContentChecked, AfterViewInit, ChangeDetectorRef, Component, inject, model, OnInit, signal, ViewChild } from '@angular/core';
...
export class ExtractRequestComponent implements OnInit, AfterViewInit, AfterContentChecked {
  ...
  searchByDate() {
    console.log( "searchByDate startDate= ", this.dateRange.get('startDate')?.value);
    console.log( "searchByDate endDate= ", this.dateRange.get('endDate')?.value);
    this.dataService.getExtractRequests().subscribe((data) => {
      this.dataSource.data = data;
    });
  }
